...
class Data:
    ...
    def readData(data, path, customerNum):
        ...
        data.cor_X.append(float(data.cor_X[0]))
        data.cor_Y.append(float(data.cor_Y[0]))
        ...
        for i in range(0,data.nodeNum):
            for j in range(0,data.nodeNum):
                ...
                data.disMatrix[i][j]=round(math.sqrt(temp),2)
                temp=0
...
for i in range(data.nodeNum-1):
    ...
    Graph.add_node(name
                   ...
                   ,serviceTime=data.serviceTime[i]
                   ,x_coor=X_coor
                   ,y_coor=Y_coor
                   ,min_dis=0
                   ,previous_node=None
...
# we want to set a different coor for node (n+1), by the way (n+1)=(data.nodeNum-1)

#----stop at page 165 line 174
