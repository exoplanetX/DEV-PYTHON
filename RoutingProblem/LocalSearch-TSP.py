...
for i in range(max_iteration):
    ...
    delta = calc_delta(current_route, index_i, index_j)
    if delta<0:
        current_route=two_opt_operator(current_route,index_i,index_j)
        total+=delta
    #print("selected index are: %d,%d"%(index_i,index_j))
    #print("the %d th iteration distance is %.4f"%(i,total))
end=time.time()
duration=end-start
